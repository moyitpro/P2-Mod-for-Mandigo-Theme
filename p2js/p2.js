...
jQuery(function($) {
		...
		ajaxCheckPosts = $.getJSON(queryString, function(newPosts){
			if (newPosts != null) {
				...
				if (!isFirstFrontPage || (typeof newPosts.html == "undefined") ) {
					...
					$("#p2main > ul > li:first").before(newPosts.html);
					var newUpdatesLi = $("#p2main > ul > li:first");
					...
					if (counter >= newPosts.numberofnewposts && showNotification) {
						var updatemsg = isElementVisible('#p2main > ul >li:first') ? "" :  "<a href=\"#\"  onclick=\"jumpToTop();\" \">"+p2txt.jump_to_top+"</a>" ;
	...
	*/
	...
	function removeYellow() {
		...
		if (isFirstFrontPage) {
			$('#p2main > ul > li.newupdates').each(function() {
				if (isElementVisible(this)) {
